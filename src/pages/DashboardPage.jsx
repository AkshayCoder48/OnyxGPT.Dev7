    ...
    <div className="min-h-screen bg-[#050505] flex text-white font-sans selection:bg-primary/30">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#0a0a0a] flex flex-col shrink-0">
        ...
        <nav className="flex-1 px-6 mt-4 space-y-1">
          ...
          <SidebarButton active={activeTab === 'activity'} onClick={() => setActiveTab('activity')} icon={<Activity size={18} />} label="Recent Logs" />
          <SidebarButton active={activeTab === 'github'} onClick={() => setActiveTab('github')} icon={<Github size={18} />} label="GitHub Hub" />
        </nav>

        <div className="p-8">
          <div className="p-5 bg-white/[0.03] border border-white/5 rounded-3xl backdrop-blur-sm">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="h-24 border-b border-white/5 flex items-center justify-between px-12 bg-black/20 backdrop-blur-2xl sticky top-0 z-10">
          <div className="flex flex-col">
             ...
             <div className="text-[10px] text-primary/40 font-mono mt-1">status: operational // latency: 24ms</div>
          </div>
          ...
          <div className="flex items-center space-x-6">
            <div className="relative hidden lg:block">
              ...
              <input
                type="text"
                placeholder="Query sandbox..."
                className="bg-white/5 border border-white/10 rounded-2xl pl-12 pr-6 py-3 text-xs outline-none focus:border-primary/50 focus:bg-white/[0.08] transition-all w-80 font-mono"
                value={searchQuery}
            ...
            <button
              ...
              className="bg-primary text-black font-black px-8 py-3 rounded-2xl text-xs hover:shadow-[0_0_30px_rgba(0,228,204,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center space-x-3"
            >
          ...
          {activeTab === 'projects' && (
            <div className="space-y-12">
               ...
               <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <StatCard icon={<Cpu size={16}/>} label="Compute Cluster" value="Global" />
                  <StatCard icon={<Globe size={16}/>} label="Sync Status" value="Active" color="text-primary" />
                  <StatCard icon={<Zap size={16}/>} label="Efficiency" value="99.9%" />
               </div>
          ...
          {activeTab === 'activity' && (
            <div className="max-w-4xl mx-auto space-y-8">
               <div className="flex items-center space-x-4 mb-10">
                  ...
                  <h3 className="text-3xl font-display font-bold"> Neural Output Logs</h3>
               </div>
               {[1,2,3,4,5].map(i => (
                 <div key={i} className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl flex items-center justify-between group hover:border-primary/30 hover:bg-white/[0.04] transition-all">
                    <div className="flex items-center space-x-6">
                       ...
                       <div>
                          <div className="font-bold text-base text-gray-200">Filesystem synchronized with Puter Cloud</div>
                          <div className="text-[10px] text-gray-600 font-mono mt-1 uppercase tracking-widest">Instance ID: 0xFD445 • TS: {Date.now() - (i * 3600000)}ms</div>
                       </div>
                    </div>
                    <div className="px-3 py-1 bg-primary/10 border border-primary/20 rounded-full text-[9px] font-black text-primary uppercase">Sync_OK</div>
                 </div>
               ))}
               <div className="text-center py-16 opacity-30">
                  <div className="w-20 h-1 bg-white/10 mx-auto mb-4 rounded-full"></div>
                  <p className="text-gray-600 text-xs font-mono">END OF TRACE</p>
               </div>
            </div>
          )}

          {activeTab === 'github' && (
            <div className="max-w-3xl mx-auto bg-[#0a0a0a] border border-white/5 rounded-[4rem] p-16 text-center relative overflow-hidden shadow-2xl">
              ...
              <div className="relative">
                 <div className="w-24 h-24 bg-white/5 rounded-[2.5rem] flex items-center justify-center mx-auto mb-10 border border-white/10">
                    <Github size={48} className="text-white" />
                 </div>
                 {isGitHubConnected ? (
                   <div className="space-y-10">
                      <div className="flex items-center justify-center space-x-6">
                         {ghUser?.avatar_url && <img src={ghUser.avatar_url} className="w-24 h-24 rounded-[2rem] border-4 border-[#050505] shadow-[0_0_50px_rgba(0,228,204,0.2)]" alt="GH Avatar" />}
                         <div className="text-left">
                           <div className="flex items-center space-x-3 text-white font-bold text-3xl">
                             <CheckCircle2 size={32} className="text-primary" />
                             <span>{ghUser?.login}</span>
                           </div>
                           <p className="text-gray-500 text-sm mt-2 max-w-xs">{ghUser?.bio || 'Neural infrastructure developer'}</p>
                         </div>
                      </div>
                      <div className="grid grid-cols-3 gap-6">
                         <DetailCard label="Public Projects" value={ghUser?.public_repos || 0} />
                         <DetailCard label="Gists" value={ghUser?.public_gists || 0} />
                         <DetailCard label="Network" value={ghUser?.followers || 0} />
                      </div>
                      <button
                       onClick={async () => {
                   ...
                   <div className="space-y-8">
                     ...
                     <button
                       ...
                       className="bg-white text-black font-black px-12 py-5 rounded-3xl hover:scale-105 transition-all shadow-[0_20px_40px_rgba(255,255,255,0.1)] flex items-center space-x-4 mx-auto active:scale-95"
                     >
                       <Github size={24} />
                       <span className="uppercase tracking-widest text-sm">Authorize Hub</span>
                     </button>
                     <p className="text-[10px] text-gray-700 font-mono">Neural tokens are encrypted via Puter AES-256</p>
                   </div>
                 )}
              </div>
            </div>
...
function StatCard({ icon, label, value, color = "text-white" }) {
   return (
      <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2rem] flex flex-col space-y-6 hover:bg-white/[0.04] transition-all group">
         <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-gray-500 group-hover:text-primary transition-colors border border-white/5">
            {icon}
         </div>
         <div>
...
function SidebarButton({ active, onClick, icon, label }) {
  return (
    ...
    >
      {active && <div className="absolute left-[-24px] w-2 h-8 bg-primary rounded-full shadow-[0_0_15px_rgba(0,228,204,0.6)]"></div>}
...
function ProjectCard({ project, onClick, onDelete }) {
  ...
  return (
    <div
      className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] overflow-hidden group hover:border-primary/40 hover:bg-white/[0.04] transition-all cursor-pointer relative shadow-xl"
      onClick={onClick}
    >
      <div className="h-40 bg-black/60 flex items-center justify-center relative overflow-hidden">
        ...
        <Folder size={64} className="text-white/5 group-hover:text-primary/40 group-hover:scale-110 transition-all duration-700 relative z-10" />
      </div>
      <div className="p-8 flex items-center justify-between">
        ...
        <div className="relative">
          <button
            ...
            className="p-3 text-gray-600 hover:text-white hover:bg-white/10 rounded-2xl transition-all"
          >
            <MoreVertical size={20} />
          </button>

          {showMenu && (
            <div className="absolute right-0 bottom-full mb-4 w-48 bg-[#0a0a0a]/90 border border-white/10 rounded-3xl shadow-2xl z-20 overflow-hidden backdrop-blur-3xl animate-in fade-in slide-in-from-bottom-4 duration-300">
               <MenuButton icon={<Pin size={16} />} label="Anchor Instance" />
               <MenuButton icon={<Download size={16} />} label="Export Config" />
...
function MenuButton({ icon, label, danger, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center space-x-4 px-5 py-4 text-[11px] font-black uppercase tracking-widest transition-all ${
